...
void inc_indent() { g_indent++; }

void dec_indent() { g_indent--; }
...
static const char *DEFAULT_REG_NAMES[7] = {"a",  "b",  "c", "d",
                                           "bp", "sp", "pc"};
...
const char *src_str(Inst *inst) { return value_str(&inst->src); }

Op normalize_cond(Op op, bool flip) {
  if (op >= 16)
    op -= 8;
  if (flip) {
    static const Op TBL[] = {JNE, JEQ, JGE, JLE, JGT, JLT, JMP};
    op = TBL[op - JEQ];
  }
  return (Op)op;
}
...
int emit_cnt() { return g_emit_cnt; }
...
void emit_start() { g_emit_started = true; }
...
/* Split functions by 24-bit PC windows */
int CHUNKED_FUNC_SIZE = 1 << 24;

int emit_chunked_main_loop(Inst *inst, void (*emit_func_prologue)(int func_id),
  ...
  for (; inst; inst = inst->next) {
    int abs_pc = inst->pc;
    int func_id = abs_pc / CHUNKED_FUNC_SIZE;  /* high bits = function */
    int local_pc = abs_pc % CHUNKED_FUNC_SIZE; /* low  24 bits = BB idx */

    if (prev_abs_pc != abs_pc) {
      ...
      /* announce BB change using the local PC within the function */
      emit_pc_change(local_pc);
    }
